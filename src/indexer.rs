...
impl Indexer {
    ...
    fn sync_transactions(
        ...
        let verification_tip = if !self.fixed_listsinceblock {
        ...
        let since_block = self.tip.as_ref().map(|tip| tip.hash());
        let result = self.rpc.list_since_block_(since_block)?;

        // Ensure consistent results for `listsinceblock` in < 0.21.
        if verification_tip.map_or(false, |vtip| result.lastblock != *vtip.hash()) {
        ...
        let tip = verification_tip.map_or_else(
            || -> Result<_> {
                if since_block != Some(&result.lastblock) {
